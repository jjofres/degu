...
def run_script(EM):
    ...
    resdata['variation_properties'] = {
        'Young': {'min': minf_Y, 'max': maxf_Y, 'anisotropy': anis_Y, 'min_direction': cd_min_Y,
                  'max_direction': cd_max_Y},
        'LinearCompressibility': {'min': minf_LC, 'max': maxf_LC, 'anisotropy': anis_LC, 'min_direction': cd_min_LC,
                                  'max_direction': cd_max_LC},
        'Shear': {'min': minf_S0, 'max': maxf_S0, 'anisotropy': anis_S0, 'min_direction': cd_min_S0,
                  'max_direction': cd_max_S0},
        'Shear2': {'min': minf_S1, 'max': maxf_S1, 'anisotropy': anis_S1, 'min_direction': cd_min_S1,
                   'max_direction': cd_max_S1},
        'Poisson': {'min': minf_P1, 'max': maxf_P1, 'anisotropy': anis_P1, 'min_direction': cd_min_P1,
                    'max_direction': cd_max_P1},
        'Poisson2': {'min': minf_P2, 'max': maxf_P2, 'anisotropy': anis_P2, 'min_direction': cd_min_P2,
                     'max_direction': cd_max_P2}
        }
...
def run_script_plots(axs, EM):
    ...
    axY[0].text(5 / 4 * np.pi, axY[0].get_rmax() * 1.7, 'xy plane', fontsize=10, ha='left')
    axL[0].text(5 / 4 * np.pi, axL[0].get_rmax() * 1.7, 'xy plane', fontsize=10, ha='left')
    axS[0].text(5 / 4 * np.pi, axS[0].get_rmax() * 1.7, 'xy plane', fontsize=10, ha='left')
    axP[0].text(5 / 4 * np.pi, axP[0].get_rmax() * 1.7, 'xy plane', fontsize=10, ha='left')

    axY[1].text(5 / 4 * np.pi, axY[1].get_rmax() * 1.7, 'xz plane', fontsize=10, ha='left')
    axL[1].text(5 / 4 * np.pi, axL[1].get_rmax() * 1.7, 'xz plane', fontsize=10, ha='left')
    axS[1].text(5 / 4 * np.pi, axS[1].get_rmax() * 1.7, 'xz plane', fontsize=10, ha='left')
    axP[1].text(5 / 4 * np.pi, axP[1].get_rmax() * 1.7, 'xz plane', fontsize=10, ha='left')

    axY[2].text(5 / 4 * np.pi, axY[2].get_rmax() * 1.7, 'yz plane', fontsize=10, ha='left')
    axL[2].text(5 / 4 * np.pi, axL[2].get_rmax() * 1.7, 'yz plane', fontsize=10, ha='left')
    axS[2].text(5 / 4 * np.pi, axS[2].get_rmax() * 1.7, 'yz plane', fontsize=10, ha='left')
    axP[2].text(5 / 4 * np.pi, axP[2].get_rmax() * 1.7, 'yz plane', fontsize=10, ha='left')

    axY[0].set_title("Young's modulus (GPa)")
    axL[0].set_title("Linear compressibility (TPa$^{-1}$)")
    axS[0].set_title("Shear modulus (GPa)")
    axP[0].set_title("Poisson's ratio")
    # plt.show()
